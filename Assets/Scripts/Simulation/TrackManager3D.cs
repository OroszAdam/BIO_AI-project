...
/// </summary>
public class TrackManager3D : MonoBehaviour
{
    #region Members
    public static TrackManager3D Instance
    {
        get;
        private set;
    }
    ...
    /// </summary>
    public CarController3D PrototypeCar;
    ...
    {
        public RaceCar(CarController3D car = null, uint checkpointIndex = 1)
        {
            this.Car = car;
            this.CheckpointIndex = checkpointIndex;
        }
        public CarController3D Car;
    ...
    private CarController3D bestCar = null;
    /// <summary>
    /// The current best car (furthest in the track).
    /// </summary>
    public CarController3D BestCar
    {
        get { return bestCar; }
        private set
        {
            if (bestCar != value)
            {
                ...
                CarController3D previousBest = bestCar;
                bestCar = value;
    ...
    /// </summary>
    public event System.Action<CarController3D> BestCarChanged;

    private CarController3D secondBestCar = null;
    /// <summary>
    /// The current second best car (furthest in the track).
    /// </summary>
    public CarController3D SecondBestCar
    {
    ...
    /// </summary>
    public event System.Action<CarController3D> SecondBestCarChanged;
    #endregion
    ...
    public void SetCarAmount(int amount)
    {
        ...
        if (amount > cars.Count)
        {
            ...
            for (int toBeAdded = amount - cars.Count; toBeAdded > 0; toBeAdded--)
            {
                ...
                CarController3D controllerCopy = carCopy.GetComponent<CarController3D>();
    ...
    /// <summary>
    /// Instantiate a car that the user can drive.
    /// <summary>
    public void InstantiateRaceCar()
    {
        GameObject carCopy = Instantiate(PrototypeCar.gameObject);
        carCopy.transform.position = startPosition;
        carCopy.transform.rotation = startRotation;
        CarController3D controllerCopy = carCopy.GetComponent<CarController3D>();
        controllerCopy.UseUserInput = true;
        GameStateManager3D.Instance.carPlayerControlled = carCopy;
        carCopy.SetActive(true);
    }
    ...
    public void Restart()
    {
        ...
        /* if (BestCar != null)
        Debug.Log("BestCar   ["+ BestCar.name +"].raceTime: " + BestCar.raceTime); */
    ...
    /// </summary>
    public IEnumerator<CarController3D> GetCarEnumerator()
    {
        for (int i = 0; i < cars.Count; i++)
    ...
    private float GetCompletePerc(CarController3D car, ref uint curCheckpointIndex)
    {
        ...
        //Debug.Log("car: " + car + ", distance from checkpoint[" + curCheckpointIndex + "]= " + Vector2.Distance(car.transform.position, checkpoints[curCheckpointIndex].transform.position));
